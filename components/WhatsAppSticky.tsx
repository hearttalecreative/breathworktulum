...
// Delicate, quiet — a small refined disc with a soft shadow and a thin ring,
// gentle press on tap. No attention-grabbing pulse.
export default function WhatsAppSticky() {
  return (
    <a
      ...
      className="group fixed bottom-[max(1.1rem,env(safe-area-inset-bottom))] right-4 z-40 flex h-12 w-12 items-center justify-center rounded-full bg-whatsapp text-pure shadow-[0_10px_30px_-8px_rgba(25,27,23,0.45)] ring-1 ring-inset ring-pure/20 transition-transform duration-300 active:scale-95 lg:hidden"
    >
      {/* Soft halo so it sits on the page, never harsh. */}
      <span
        aria-hidden
        className="pointer-events-none absolute inset-0 rounded-full ring-1 ring-whatsapp/30"
